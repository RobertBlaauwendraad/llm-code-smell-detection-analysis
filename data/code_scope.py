class CodeScope:
    ...
    @staticmethod
    def get_by_id_and_types(conn, sample_id, scope_types):
        cursor = conn.cursor()
        cursor.execute('''
            SELECT * FROM CodeScope WHERE sample_id = ? AND scope_type IN ({})
        '''.format(','.join('?' * len(scope_types)), scope_types), (sample_id, *scope_types))
        result = cursor.fetchone()
        if result:
            code_scope = CodeScope(result[1], result[2], result[3])
            code_scope.id = result[0]
